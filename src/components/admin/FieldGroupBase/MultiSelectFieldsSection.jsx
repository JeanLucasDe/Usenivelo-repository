...
export default function MultiSelectFieldsSection({
  camposPorTipo,
  editingFieldId,
  newField,
  creatingFieldType,
  ...
  if (!camposPorTipo.multiselect) return null;

  const handleEditSubField = (campo, sub) => {
    ...
    <div className="grid select-none bg-white rounded-xl border border-gray-400 space-y-5 shadow-lg w-full">
      <div className="w-full">
        <div className="flex p-4 items-center">
          <h3 className="text-gray-700 font-semibold ml-3">MultiSelect</h3>
          <button
        ...
        <div className="grid gap-2 w-full">
          {camposPorTipo.multiselect.map((campo) => (
            <div key={campo.id} className="w-full mb-4 relative">
              {/* Campo principal */}
              <div className="flex items-center mt-2 mb-2 ml-2 justify-between">
                <div className="flex items-center w-full">
                  {editingFieldId === campo.id && !newField.editSubField ? (
                    <>
                      <input
                ...
                {/* Menu de ações */}
                {showMenu === campo.id && (
                  <div className="absolute right-0 mt-2 w-44 bg-white border border-gray-300 rounded-lg shadow-lg z-100">
                    <button
                      onClick={() => {
                        handleNewClick('multiselect');
                    ...
                    <button
                      onClick={() => { handleEditClick(campo); setShowMenu(null); }}
                      className="block w-full text-left px-3 py-2 text-sm hover:bg-gray-100 text-blue-600"
                    >
                      Editar Nome
                    </button>
                    <button
                      onClick={() => { handleDeleteField(campo); setShowMenu(null); }}
                ...
                <div className="mb-2 px-2">
                  <label className="flex items-center text-gray-700 dark:text-gray-200">
                    ...
                    <input
                      type="number"
                      min={1}
                      max={subFields.filter(sf => sf.field_id === campo.id).length || 1}
                      value={newField.limit || ''}
                      onChange={(e) => {
                        const value = e.target.value ? parseInt(e.target.value) : 1;
                        const maxOptions = subFields.filter(sf => sf.field_id === campo.id).length || 1;
                  ...
                  {newField.limit > (subFields.filter(sf => sf.field_id === campo.id).length || 0) && (
                    <p className="text-sm text-red-600 mt-1">
                      O limite não pode ser maior que o número de opções ({subFields.filter(sf => sf.field_id === campo.id).length})
                    </p>
                  )}
                </div>
              )}
              ...
              {/* Novo subcampo */}
              {creatingFieldType === 'multiselect' && campo.id === selectSubCamp && (
                ...
                .map((sub) => (
                  ...
                  >
                    <span className="flex items-center w-full">
                      {editingFieldId === sub.id && newField.editSubField ? (
                        <>
                          <input
                            className="border border-gray-400 rounded-md px-2 py-1 text-sm w-full"
                            value={newField.name || sub.name}
                            onChange={(e) => setNewField({ ...newField, name: e.target.value })}
                          />
                          <button
                        ...
                        <>
                          ...
                          <Edit
                            ...
                            onClick={() => handleEditSubField(campo, sub)}
                          />
                        </>
                      )}
                    </span>
